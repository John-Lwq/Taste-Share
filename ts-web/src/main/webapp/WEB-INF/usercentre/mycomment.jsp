...
<c:forEach  begin="0" end="5" step="1" items="${restaurantList}" var="item">
    <span data-hook="review-body" class="a-size-base review-text">
    <font size="3"  face="宋体">商家名称：${item.shopName}
    <br>
             商家地址：${item.shopName}
    <br>
     <c:forEach  begin="0" end="10" step="1" items="${restaurantDescList}" var="ditem">
              我的评论：<c:if test="${item.restaurantId==ditem.restaurantId}">
              ${ditem.comment }
              </c:if>
     </c:forEach>
    <br>
